...
class ApplicationController @Inject()(cc: ControllerComponents, userService: UserService) extends AbstractController(cc) with Logging {
  ...
  def updateUser() = Action.async { implicit request: Request[AnyContent] =>
    UserForm.form2.bindFromRequest.fold(
      // if any error in submitted data
      errorForm => {
        logger.warn(s"Form submission with error: ${errorForm.errors}")
        Future.successful(Ok(views.html.userDetay(errorForm, Option.empty[User])))
      },
      data => {
        val newUser = User(data.id, data.firstName, data.lastName, data.mobile, data.email)
        userService.updateUserAllCoumn(newUser).map( _ => Redirect("/update/"+data.id))
      })
  }
  ...
  def getUser(id: Long) = Action.async { implicit request: Request[AnyContent] =>
    userService.getUser(id) map { user =>
      Ok(views.html.userDetay(UserForm.form2, user))
    }
  }
